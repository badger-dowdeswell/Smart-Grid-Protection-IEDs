...
const TDataIOID FORTE_earth_fault_5::scm_anEOWith[] = {0, 255, 0, 1, 255};
const TForteInt16 FORTE_earth_fault_5::scm_anEOWithIndexes[] = {0, 2, -1, -1};
  ...
  do{
    ...
    switch(m_nECCState){
      ...
      case scm_nStateSTATE1:
        cout << "executeEvent() STATE1 voltage=" << to_string(voltage()) << " pickup_voltage=" << to_string(pickup_voltage()) << "\n";
        if ( ((scm_nEventREQID == pa_nEIID) && (voltage() < pickup_voltage())) ||  ((scm_nEventREQID == pa_nEIID) && (voltage() == pickup_voltage())) )          
          enterStateSTATE3();
        else
      ...
      case scm_nStateSTATE4:
        cout << "executeEvent() STATE4 voltage=" << to_string(voltage()) << " pickup_voltage=" << to_string(pickup_voltage()) <<"\n";
        if ( (scm_nEventREQID == pa_nEIID) && (voltage() > pickup_voltage()) )
          enterStateSTATE1();
        else
        if(scm_nEventRESETID == pa_nEIID)
          enterStateSTATE5();
